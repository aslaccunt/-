...
calculatePrepayment() {
    ...
    // ✅ extras رو به صورت object کامل دربیار
    const selectedExtras = (this.data?.extras || [])
        .filter(extra => this.selections.extras.has(extra.id))
        .map(extra => ({
            id: extra.id,
            label: extra.label || extra.title || extra.id,
    ...
    return {
        ...
        bookingTypeLabel: this._getBookingTypeLabel(),
        subOption: this.selections.subOption,
        extras: selectedExtras,   // ✅ حالا آرایه‌ای از object
...
// ✅ helper برای گرفتن label نوع برنامه
_getBookingTypeLabel() {
    const typeId = this.selections.bookingType;
    const bt = (this.data?.bookingTypes || []).find(t => t.id === typeId);
    return bt?.label || bt?.title || typeId || '';
},
};
...
async function submitBooking() {
    ...
    try {
        ...
        const res = await fetch(CONFIG.API_URL, { method: 'GET', body: formData });
