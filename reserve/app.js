...
const State = {
    ...
    // داخل State، بعد از calculateTotal
    calculateFinal() {
        const { total, breakdown } = this.calculateTotal();
        const discount = Discount.apply(total);
        return {
            originalTotal: total,
            finalTotal: discount.final,
    ...
    calculatePrepayment() {
        const { finalTotal } = this.calculateFinal();
        const isVip = String(this.data?.profile?.type || '').toUpperCase() === 'VIP';

        if (!isVip) {
            return { isVip: false, prepayment: 0, remaining: finalTotal };
        }
        ...
        const prepayment = FIXED + Math.round((finalTotal - FIXED) * PERCENT);
        const remaining = finalTotal - prepayment;

        return { isVip: true, prepayment, remaining };
    },

    buildPayload() {
        const { originalTotal, finalTotal, discountApplied, breakdown } = this.calculateFinal();
...
const Discount = {
    list: [], // لیست تخفیف دارها از data.json
    ...
    async load() {
        try {
            ...
            this.list = json.discount || [];
        } catch (e) {
            console.warn('⚠️ Discount list load failed:', e);
            this.list = [];
        }
    },
    ...
    isEligible(code) {
        ...
        return this.list.some(d => String(d.code) === String(code));
    },

    /**
     * @param {number} total - جمع کل قبل از تخفیف
     * @returns {{ applied: boolean, original: number, final: number, rule: string }}
     */
    apply(total) {
        if (!this.isEligible(State.code)) {
            return { applied: false, original: total, final: total, rule: '' };
        }

        let final = total;
        let rule = '';

        if (total >= 1_000_000 && total < 3_000_000) {
            final = 1_000_000;
            rule = '۱ تا ۳ میلیون → ۱ میلیون';
        } else if (total >= 3_000_000 && total < 5_000_000) {
            final = 2_000_000;
            rule = '۳ تا ۵ میلیون → ۲ میلیون';
        } else if (total >= 5_000_000 && total < 7_000_000) {
            final = Math.round(total * 0.6);
            rule = '۵ تا ۷ میلیون → ۴۰٪ تخفیف';
        } else {
            return { applied: false, original: total, final: total, rule: '' };
        }

        return { applied: true, original: total, final, rule };
    },
};
...
const UI = {
    ...
    renderProfile(profile, code) {
        ...
        avatarEl.innerHTML = `<img src="${CONFIG.IMAGE_PATH}/place.png" alt="${escapeHTML(name)}" onerror="this.style.display='none'">${statusBadge}`;
...
fileInput.addEventListener('change', () => {
    ...
    renderTotal() {
        ...
        const { originalTotal, finalTotal, discountApplied, breakdown } = State.calculateFinal();
        ...
        // ردیف تخفیف — فقط متن، بدون مبلغ
        const discountHTML = discountApplied ? `
            <div class="breakdown-row discount-row">
                <span class="breakdown-label discount-label">
                    ${ICONS.check} تخفیف امروز کسر شد
                </span>
            </div>
        ` : '';
        ...
        container.innerHTML = `
            ...
            <div class="total-display">
                ...
                <div class="total-row">
                    ...
                    <span class="total-value">${formatPrice(finalTotal)} ${CONFIG.CURRENCY}</span>
                </div>
                ${prepaymentHTML}
            </div>
        `;
...
function redirectToNextPage(payload) {
    ...
    const params = new URLSearchParams({
        ...
        remaining: remaining,
        discount: payload.discountApplied ? '1' : '0',
    });
